...
/// Unified with Entity as a proper graph node: has id, semantic_features, embedding.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl RPGraph {
    ...
    /// Recompute metadata from current state.
        ...
        for (area_name, node) in &self.hierarchy {
            ...
            let lifted = ids
                ...
                .filter(|id| {
                    self.entities
                        .get(*id)
                        .map(|e| !e.semantic_features.is_empty())
                        .unwrap_or(false)
                })
                .count();
        ...
        for id in &entity_ids {
            ...
            let path = match components.len() {
                ...
                _ => {
                    ...
                    let stem = components.last().unwrap();
    ...
    /// Assign IDs to all hierarchy nodes.
